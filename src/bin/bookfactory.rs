...
fn main() {
    ...
    let file = match recipes.iter().find(|recipe| recipe.name == args.recipe_name) {
        None => panic!("Recipe {} not found in file {}.", args.recipe_name, args.config_file),
        Some(recipe) => match get_format(recipe) {
            Format::Epub2 => build_epub2(recipe).unwrap(),
            Format::Unrecognized => panic!("Format {} not recognized in recipe {}", recipe.format, recipe.name)
        }
    };
