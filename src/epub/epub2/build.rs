...
fn check_no_duplicate_inside_paths(paths_vec: &Vec<(&str, PathBuf)>) -> Result<(), String> {
    let mut inside_paths = paths_vec.iter().map(|(_outside, inside)| match inside.to_str() {
...
fn check_inside_path_is_valid(path: &PathBuf) -> Result<(), String> {
    match path.file_name() {
        ...
        Some(filename) => match filename.to_str() {
            ...
            Some(filename_str) => if filename_str.len() > 255 {
                return Err(format!("Invalid filename of length >255 bytes: {}", filename_str))
            },
        },
    };
    ...
    match path.to_str() {
        ...
        Some(path_str) => if path_str.len() > 65535 {
    ...
    for component in path.iter() {
        match component.to_str() {
            None => return Err(format!("Invalid non-UTF-8 path component (you shouldn't ever see this error): {:?}", component)),
            Some(component_str) => {
                match component_str.find(&['/', '"', '*', ':', '<', '>', '?', '\\'][..]) {
                    None => (),
                    Some(index) => return Err(format!("Path {:?} contains invalid character '{}'.", path, component_str.get(index..index + 1).unwrap())),
                };
...
pub fn build_epub2(recipe: &Recipe) -> Result<Vec<u8>, String> {
    ...
    let (add_opf_to_rootfiles, opf_path) = match &config.rootfiles {
        ...
        Some(rootfiles_vec) => match rootfiles_vec.iter().find(|rootfile| &rootfile.media_type == "application/oebps-package+xml") {
            None => (true, "OEBPS/content.opf"),
    ...
    let (ncx_id, ncx_path_from_opf) = match &config.ncx_meta { // In the long run, put more integrity-checking here, ensuring no collision with other manifest IDs and paths. (And do the same for IDs within the manifest with one another.)
        Some(meta) => {
    ...
    if let Some(nonmanifest_file_vec) = &config.nonmanifest_files {
        for nonmanifest_file in nonmanifest_file_vec {
            outside_and_inside_paths.push((nonmanifest_file.outside_path.as_ref(), PathBuf::from(&nonmanifest_file.inside_path)));
        }
    }
